...
async function getLiveSheetData() {
    ...
    console.log('[CSV Debug] Raw headers from API:', headers);
    console.log('[CSV Debug] Sample row 1 (index 1):', data.values[1]);
    // Find a row with actual Major/Year data (skip header row)
    const rowWithData = data.values.slice(1).find(row => row[5] && row[5].trim() !== '');
    if (rowWithData) {
        const rowIndex = data.values.indexOf(rowWithData);
        console.log('[CSV Debug] Row with Major/Year data (row index ' + rowIndex + '):', rowWithData);
        console.log('[CSV Debug] Major (index 5):', rowWithData[5]);
        console.log('[CSV Debug] Year (index 6):', rowWithData[6]);
        console.log('[CSV Debug] National Dues (index 7):', rowWithData[7]);
    }
    const processedData = [];
    ...
    for (let i = 1; i < data.values.length; i++) {
        ...
        if (entry['Email Address'] && entry['Email Address'].trim()) {
            ...
            // Debug: Log an entry with demographics
            if (entry['Major'] && entry['Major'].trim() !== '' && !processedData.some(e => e['Major'])) {
                console.log('[CSV Debug] First processed entry with demographics:', {
                    email: entry['Email Address'],
                    major: entry['Major'],
                    year: entry['Year'],
                    nationalDues: entry['Paid National Dues? (Note this due is separate from the chapter dues and required for conferences & scholarships)']
                });
            }
...
async function calculateMemberPoints(signInData) {
    ...
    for (const entry of signInData) {
        ...
        if (!memberStats[Object.keys(memberStats)[0]]) {
            console.log('[Demographics Debug] First sign-in entry fields:', Object.keys(entry));
            console.log('[Demographics Debug] Major field value:', entry['Major']);
            console.log('[Demographics Debug] Year field value:', entry['Year']);
            console.log('[Demographics Debug] Full entry data:', entry);
        }
        ...
        if (!memberStats[normalizedUniqname]) {
            ...
            // Debug: Log member creation with demographics
            if (major && major.trim() !== '') {
                console.log('[Member Debug] Creating member with demographics:', {
                    uniqname: normalizedUniqname,
                    major: major,
                    year: year,
                    nationalDues: nationalDues,
                    entryMajor: entry['Major'],
