...
function MentorshipHub({ onBackClick, userData, eventData, localDataManager }) {
  ...
  const calculateBadgeProgress = (badge, userStats, userActivities, allMemberPoints = [], tierThresholds = null) => {
    ...
    // Check for badge progress override first
    const badgeOverrides = window.LocalDataManager?.getBadgeOverrides() || {};
    const userEmail = userStats.email || (userActivities[0] && userActivities[0].rawEntry && userActivities[0].rawEntry['Email Address']);
    const override = badgeOverrides[userEmail] && badgeOverrides[userEmail][badge.id];
    
    if (override) {
      // Use override value
      current = override.progress;
      ...
      switch (badge.type) {
        ...
        case 'points':
          ...
          // Always use fixed badge requirements for achievement badges
