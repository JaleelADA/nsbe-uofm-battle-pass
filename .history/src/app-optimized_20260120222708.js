...
// Badge Progress Tracker - integrated into main stats page
function BadgeProgressTracker({ userName }) {
  ...
  const handleMemberSearch = async (query = searchQuery) => {
    ...
    try {
      ...
      if (memberStats) {
        ...
        setMemberBadges({ member: query, badges: [], stats: null, notFound: true });
      }
    } catch (error) {
  ...
  return (
    <StyledSection style={{
        ...
        <div className="mb-4">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 p-2 sm:p-3 bg-gray-800 border border-gray-600 text-white placeholder-gray-400 text-sm"
              style={{ clipPath: SHARED_STYLES.clipPaths.card }}
      ...
      {memberBadges && (
        <div>
          {memberBadges.notFound ? (
            <div className="text-center p-4 bg-red-900/30 border border-red-500" style={{ clipPath: SHARED_STYLES.clipPaths.card }}>
              <div className="text-red-300 text-sm">Member "{memberBadges.member}" not found.</div>
            </div>
          ) : (
            <div>
              {/* Progress Summary */}
              <div className="mb-4 p-3 bg-gradient-to-r from-yellow-900/30 to-orange-900/30 border border-yellow-400/50" style={{ clipPath: SHARED_STYLES.clipPaths.card }}>
                ...
                {memberBadges.badges.map((badge, index) => (
                  <div
                    ...
                    className={`p-3 border transition-all duration-300 ${
                      badge.earned 
                        ? 'bg-gradient-to-r from-yellow-900/40 to-orange-900/40 border-yellow-400' 
                        : badge.progress > 0
                        ? 'bg-gradient-to-r from-blue-900/30 to-cyan-900/30 border-blue-400/60'
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <span className="text-xl" style={{
                          filter: badge.earned ? 'none' : badge.progress > 0 ? 'none' : 'grayscale(1) brightness(0.6)'
                        }}>{badge.icon}</span>
                        <div>
                          <div className={`font-bold text-sm ${
                            badge.earned ? 'text-yellow-200' : badge.progress > 0 ? 'text-blue-200' : 'text-gray-400'
                          }`}>{badge.name}</div>
                      ...
                      <div className="text-right">
                        {badge.earned ? (
                          <div className="text-green-400 text-xs font-bold">✅ +{badge.xp}XP</div>
                        ) : (
                          <div className="text-xs text-gray-400">{Math.round(badge.progress * 100)}%</div>
                        )}
                      </div>
                    </div>
                    {/* Progress Bar */}
                      ...
                      <div className="mt-2 w-full bg-gray-700 h-1.5 rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ 
                            ...
                            background: badge.progress > 0 ? 'linear-gradient(90deg, #3b82f6, #06b6d4)' : 'transparent'
                          }}
                        />
                      </div>
                    )}
      ...
      {/* Empty State */}
      {!memberBadges && !isLoading && (
        <div className="text-center py-6">
          <div className="text-4xl mb-3">🏆</div>
          <p className="text-gray-400 text-sm">Enter your name above to see your badge progress!</p>
        </div>
      )}
    </StyledSection>
  );
        ...
        {hasUserData ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
            ...
            <div className="lg:col-span-2">
              <BadgeSection />
              {/* Side by side panels */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-5 mb-3 sm:mb-4">
                <MemberStats 
                  userStats={userStats} 
                  userName={userName}
                ...
                <StyledSection style={{
                  ...
                  <div className="text-center">
                    ...
                    <SectionTitle theme="blue">QUICK LINKS</SectionTitle>
                    <div className="space-y-2 mt-3">
                      <a href="https://docs.google.com/forms/d/e/1FAIpQLSfXgbgogJ_yIhwgmjH6N--WQLRF2ewLf7KkJZTQSCndZCaiNQ/viewform" target="_blank" rel="noopener noreferrer" className="block p-2 text-xs sm:text-sm text-cyan-100 hover:text-cyan-200 transition-colors border border-gray-600 hover:bg-gray-800/30" style={{ clipPath: SHARED_STYLES.clipPaths.button }}>📝 Event Sign-In</a>
                      <a href="https://mynsbe.nsbe.org/s/joinprocess" target="_blank" rel="noopener noreferrer" className="block p-2 text-xs sm:text-sm text-cyan-100 hover:text-cyan-200 transition-colors border border-gray-600 hover:bg-gray-800/30" style={{ clipPath: SHARED_STYLES.clipPaths.button }}>💳 Pay Dues</a>
                    </div>
                  </div>
                </StyledSection>
              </div>
              {/* Badge Progress underneath */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-5 mb-3 sm:mb-4">
                <StyledSection style={{
                  ...
                  <div className="text-center">
                    ...
                    <SectionTitle theme="gold">MEMBER STATS</SectionTitle>
                    <p className="mt-3 text-gray-300 text-xs sm:text-sm">Enter your name above to see your rank and stats!</p>
                  </div>
                </StyledSection>
                <StyledSection style={{
                  clipPath: 'polygon(20px 0%, 100% 0%, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0% 100%, 0% 20px)',
